    ...
    if notify then
        if #newOpportunities > 0 then
            for _, opp in ipairs(newOpportunities) do
                local message = string.format("������: {ffffff}%s. �������: ����� %d. �������: ����� %d. ������: %s$",
            ...
            sampAddChatMessage("����� �����������: {ffffff}" .. #newOpportunities .. "", 0xaaaaaa)
        end
    end
...
local function drawCheapItems(cheapItems)
    ...
    imgui.Text(u8("������� ������� �������: " .. #cheapItems))
    imgui.Separator()

    for i, item in ipairs(cheapItems) do
        LavkaButton(u8(string.format("%s -%d ���������", item.itemName .. (item.enchant ~= "" and " (" .. item.enchant .. ")" or ""), item.discountPercent)), item.lavka.LavkaUid)

        imgui.Text(u8("����: " .. comma_value(item.price) .. " " .. (item.lavka.serverId == 0 and "VC$" or "SA$")))
        imgui.SameLine()
    ...
    if notify then
        if #newCheapItems > 0 then
            for _, item in ipairs(newCheapItems) do
                local message = string.format("LOWPRICE: {ffffff}%s. ����: %s %s. �������: %d ���������. ����� %d (%s) - %d ��.",
            ...
            sampAddChatMessage("�����: {ffffff}" .. #newCheapItems .. "��", 0xaaaaaa)
        end
    end
...
local function drawCheapItemsTab()
    imgui.InputInt(u8("�������"), thresholdPercent, 1, 10)
    imgui.Text(u8("������ ������� ������� ���� �� " .. thresholdPercent[0] .. " ���������"))

    imgui.Text(u8("��������� �����������:"))
    imgui.Checkbox(u8("���������� � ����� ������� �������"), notifyNewCheapItems)
    imgui.InputInt(u8("���. ������� ��� ����������� (���������)"), minDiscountThreshold, 1, 10)
    if imgui.Button(u8("��������"), imgui.ImVec2(-1, 80 * FONT_SCALE)) then
    ...
    imgui.InputTextWithHint("##avgChartQuery", u8("�������� ��������"), avgChartQuery, ffi.sizeof(avgChartQuery))
    imgui.PopItemWidth()

    if imgui.Button(u8("7 ����"), imgui.ImVec2(150 * FONT_SCALE, 0)) then
        avgChartDays[0] = 7
    end
    imgui.SameLine()
    if imgui.Button(u8("30 ����"), imgui.ImVec2(150 * FONT_SCALE, 0)) then
        avgChartDays[0] = 30
    end
    ...
    imgui.Text(u8(string.format("%s | ������: %d ����", foundName, avgChartDays[0])))
...
local function drawCourseTab()
    ...
    if imgui.Button(u8("�������"), imgui.ImVec2(170 * FONT_SCALE, 0)) then
        avgChartMode[0] = 0
    end
    imgui.SameLine()
    if imgui.Button(u8("������"), imgui.ImVec2(170 * FONT_SCALE, 0)) then
    ...
    imgui.Text(u8(string.format("%s | ������: %d ����", foundName, avgChartDays[0])))
    imgui.SameLine()
    imgui.TextColored(imgui.ImVec4(0.75, 0.80, 0.95, 1.0), u8(modeKey == "buy" and "������" or "�������"))
    local chartW = imgui.GetContentRegionAvail().x
    local chartH = 220 * FONT_SCALE
    if chartW < 260 then chartW = 260 end
    local chartPos = imgui.GetCursorScreenPos()
...
local function drawSettingsTab()
    ...
    imgui.SliderFloat(u8("������������ ����"), uiWindowAlpha, 0.70, 1.00, "%.2f")
    imgui.SliderFloat(u8("������������ ������"), uiChildAlpha, 0.20, 1.00, "%.2f")
    imgui.SliderFloat(u8("������� ����������"), uiFontScaleValue, 0.90, 1.80, "%.2f")

    imgui.NewLine()
    imgui.Text(u8("�������"))
    if imgui.Button(u8("������ ����"), imgui.ImVec2(200 * FONT_SCALE, 0)) then
    ...
    if imgui.Button(u8("�������� ����������"), imgui.ImVec2(260 * FONT_SCALE, 0)) then
        applyAppearancePreset(0)
        saveAppearanceSettings()
    end
...
local function drawArbitrageOpportunities(opportunities)
    ...
    for i, opp in ipairs(opportunities) do
        ...
        imgui.Text(u8("������:"))
        imgui.SameLine()
        imgui.Text(u8(string.format("%s %s (� -8 ��������� �� ���� �������)", comma_value(opp.profit), opp.buyLavka.serverId == 0 and "VC$" or "SA$")))
...
local function drawProfitTab()
    ...
    if imgui.InputInt(u8("���. ������ ��� �����������"), minProfitThreshold, 100, 1000) then
        ini.main.notify_min_profit = minProfitThreshold[0]
        inicfg.save(ini, directIni)
    end
    if imgui.Checkbox(u8("���������� � ����� ������������"), notifyNewArbitrage) then
...
local function drawLavkaCard(data)
    ...
    imgui.Text(u8("����� ������: " .. data.username))
    imgui.Text(u8("�������: " .. #data.items_sell))
    imgui.Text(u8("������: " .. #data.items_buy))
    imgui.Text(u8("������: " .. data.serverId))
    imgui.Text(u8("�����: " .. data.LavkaUid))
...
local function drawUserLavka(data)
    ...
    imgui.Text(u8("�������� �����: " .. data.username))
    imgui.Text(u8("ID �����: " .. data.LavkaUid))
    imgui.Text(u8("����� ��������� � �����: " .. #data.items_buy + #data.items_sell))
    imgui.Text(u8("��������� �� ������: " .. #data.items_buy))
    imgui.SameLine()
...
local function drawSearchResultsColumn(items, id)
    ...
    for k, item in pairs(items) do
        ...
        imgui.Text(u8(string.format("%s ��. | %s %s ������: %s",
...
local function drawSearchResults(data, all)
    ...
    imgui.NewLine()
    imgui.Text(u8("��������� �� ������: " .. #data.buy))
    imgui.SameLine()
    imgui.SetCursorPosX(imgui.GetWindowWidth() / 2)
    imgui.Text(u8("��������� �� �������: " .. #data.sell))
    ...
    function(self)
        ...
        if imgui.Begin(u8('###marketplace_window'), renderWindow, imgui.WindowFlags.NoCollapse) then
            ...
            if vipFeatureEnabled and not vipAuthorized then
                imgui.Text(u8("��������� VIP-����"))
                imgui.PushItemWidth(420 * FONT_SCALE)
                imgui.InputTextWithHint("##vipKey", u8("������� ����"), vipKeyInput, ffi.sizeof(vipKeyInput))
                imgui.PopItemWidth()
                if imgui.Button(u8("������������"), imgui.ImVec2(200 * FONT_SCALE, 0)) then
            ...
            if activeSidebarSection == 1 then
                ...
                if imgui.InputTextWithHint("##search", u8("����� ������"), search, ffi.sizeof(search)) then
